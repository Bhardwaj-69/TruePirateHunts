...
async def start(client:Client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        status = get_status()
        aks=await message.reply_text(f"<b>🗿yes Pirate💥{status},\nHow can i Help.🏝</b>")
        ...
        try: 
         ...
         #       newPoint = await db.get_point(refUserId)
             if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAKTlmfudEZKYYjP4l6XJZ5QRYWu00c3AAKwDwAC7osxVhzQOn1XTmwNHgQ") 
    ...
    if AUTH_CHANNEL and not await is_req_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            ...
            try:
                ...
                btn.append(
                    [InlineKeyboardButton("♻️ Try Again ♻️", callback_data=f"checksub#{file_id}#{int(grp_id)}")]
                )
            except (IndexError, ValueError):
                print('IndexError: ', IndexError)
                btn.append(
                    [InlineKeyboardButton("♻️ Try Again ♻️", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")]
                )
        reply_markup=InlineKeyboardMarkup(btn)
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener or is_third_shortener:
            ...
            buttons = [[
                ...
                InlineKeyboardButton(text="🏴‍☠️buy Premium to Skip Veify🤞", callback_data='seeplans'),
            ]]
            reply_markup=InlineKeyboardMarkup(buttons)
            if await db.user_verified(user_id): 
    ...
    if data and data.startswith("allfiles"):
        ...
        if not files:
            await message.reply_text("<b>⚠️Files not Found💢</b>")
            return
        files_to_delete = []
        for file in files:
    ...
    if not files_:
        ...
        return await message.reply('<b>⚠️Files not Found💢</b>')
    files = files_[0]
    settings = await get_settings(grp_id)
    CAPTION = settings['caption']
    f_caption = CAPTION.format(
...
async def delete(bot, message):
    ...
    if result.deleted_count:
        await msg.edit('<b>💢File Deleted from Pirate Ship💥</b>')
    else:
        ...
        if result.deleted_count:
            ...
            if result.deleted_count:
                ...
                await msg.edit('<b>⚠️Files not Found💢</b>')
...
async def settings(client, message):
    ...
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text("<code>❗Use Cmd in Grp.!⭕.</code>")
    grp_id = message.chat.id
    if not await is_check_admin(client, grp_id, message.from_user.id):
        return await message.reply_text('<b>💢Owners Only💀</b>')
    settings = await get_settings(grp_id)
    ...
    if settings is not None:
            ...
            await message.reply_text(
                text=f"Lets make Changes Pirate.🎃 <b>'{title}'</b> as all Know",
                reply_markup=InlineKeyboardMarkup(buttons),
                parse_mode=enums.ParseMode.HTML
            )
    else:
        await message.reply_text('<b>⭕Kuch to GadBad hai Pirate.!❗</b>')

@Client.on_message(filters.command('set_template'))
async def save_template(client, message):
    chat_type = message.chat.type
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text("<b>❗Use Cmd in Grp.!⭕</b>")
    grp_id = message.chat.id
    title = message.chat.title
    if not await is_check_admin(client, grp_id, message.from_user.id):
        return await message.reply_text('<b>💢Owners Only💀</b>')
...
async def send_request(bot, message):
    try:
        ...
        return
    buttons = [[
        InlineKeyboardButton('👀View Req.👀', url=f"{message.link}")
    ],[
...
async def search_files(bot, message):
    ...
    if int(total) == 0:
        await message.reply_text('<i>♨Ohho Cant Find Captain❄</i>')
        return 
    ...
    await message.reply_document(
        ...
        caption=f"<b>♻️By your  Search I Found - <code>{total}</code> Files</b>",
        parse_mode=enums.ParseMode.HTML
    )
...
async def delete_files(bot, message):
    ...
    if deleted_files_count > 0:
        await message.reply_text(f'<b>{deleted_files_count} 💢File Deleted from Pirate Ship💥</b>')
    if not_found_files:
        await message.reply_text(f'<b>💢File Deleted from Pirate Ship💥 - <code>{", ".join(not_found_files)}</code></b>')
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/LarvaLinks</code>")    
...
async def set_tutorial_2(client, message):
    ...
    chat_type = message.chat.type
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text(f"<b>❗Use Cmd in Grp.!⭕..\n\nGroup Name: {title}\nGroup ID: {grp_id}\nGroup Invite Link: {invite_link}</b>")
    try:
        tutorial = message.text.split(" ", 1)[1]
    except:
        return await message.reply_text("<b>ᴄᴏᴍᴍᴀɴᴅ ɪɴᴄᴏᴍᴘʟᴇᴛᴇ !!\n\nᴜꜱᴇ ʟɪᴋᴇ ᴛʜɪꜱ -</b>\n\n<code>/set_tutorial_2 https://t.me/LarvaLinks</code>")
...
async def set_shortner(c, m):
    ...
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await m.reply_text("<b>❗Use Cmd in Grp.!⭕..</b>")
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>💢Owners Only💀</b>')        
    if len(m.text.split()) == 1:
        await m.reply("<b>Use this command like this - \n\n`/set_shortner tnshort.net 06b24eb6bbb025fb3f696b6d5de11354`</b>")
        return        
    sts = await m.reply("<b>♻️ Checking...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/MovieWalaChat').json()
...
async def set_shortner_2(c, m):
    ...
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>💢Owners Only💀</b>')
    if len(m.text.split()) == 1:
        await m.reply("<b>Use this command like this - \n\n`/set_shortner_2 tnshort.net 06b24eb6bbb025713cd522f5de11354`</b>")
        return
    sts = await m.reply("<b>♻️ Checking...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
    try:
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/MovieWalaChat>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_2 mdiskshortner.link e7beb3c8f70bec495abc65f58c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)
...
async def set_shortner_3(c, m):
    ...
    if not userid:
        return await m.reply(f"<b>💢Owners Only💀</b>")
    grp_id = m.chat.id
    #check if user admin or not
    if not await is_check_admin(c, grp_id, userid):
        return await m.reply_text('<b>💢Owners Only💀</b>')
    if len(m.command) == 1:
        await m.reply_text("<b>ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴀᴅᴅ sʜᴏʀᴛɴᴇʀ & ᴀᴘɪ\n\nᴇx - `/set_shortner_3 mdiskshortner.link e7beb3c8f756df5abc65f58c0dfa95`</b>", quote=True)
        return
    try:
        URL = m.command[1]
        API = m.command[2]
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/MovieWalaChat>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_3 mdiskshortner.link e7beb3c8f756dfa15d0bec495abc0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)
...
async def set_log(client, message):
    ...
    if len(message.text.split()) == 1:
        ...
        return
    sts = await message.reply("<b>♻️ Checking...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
    chat_type = message.chat.type
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
    ...
    try:
        t = await client.send_message(chat_id=log, text="<b>Ahhoyy ᴡʜᴀᴛ's ᴜᴘ!! Pirate🗿</b>")
        await asyncio.sleep(3)
        await t.delete()
    except Exception as e:
...
async def most(client, callback_query):
    ...
    await callback_query.message.reply_text("<b>Most Search List..⚡ 👇</b>", reply_markup=reply_markup)
...
async def set_pm_search_on(client, message):
    ...
    await message.reply_text("<b><i>💥💢PM Search STARTED💢💥</i></b>")
...
async def set_pm_search_off(client, message):
    ...
    await message.reply_text("<b><i>❌❗PM Search Turned Offf❗.</i></b>")
...
async def set_send_movie_on(client, message):
    ...
    await message.reply_text("<b><i>💥♻Movie Update Turned Onnn♻💥</i></b>")
...
async def set_send_movie_update_off(client, message):
    ...
    await message.reply_text("<b><i>❌️❗Movie Search Disabled ❗.</i></b>")
...
async def generate_verify_id(bot, message):
    ...
    chat_type = message.chat.type
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text("❗Use Cmd in Grp.!⭕")
...
async def reset_group_command(client, message):
    ...
    await message.reply_text('⭕♻Grp Settings Reseted♻⭕...')
